...
if (isset($_SERVER["REQUEST_METHOD"]) && $_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["actualizar"])) {
    // Consulta SQL para borrar todos los registros de la tabla ResumenSS
    $truncateQuery = "TRUNCATE TABLE ResumenSS";

    // Ejecutar la consulta de borrado
    if ($conn->query($truncateQuery) === TRUE) {
        // Consulta SQL para insertar los datos actualizados en la tabla ResumenSS
        $insertQuery = "
            INSERT INTO ResumenSS (periodo, totalTurnos, totalHoras, nCuenta)
            SELECT
                CONCAT('Semana-', WEEK(FechaTurno)) AS periodo,
                COUNT(*) AS totalTurnos,
                TRUNCATE(SUM(TIMEDIFF(hSalida, hEntrada))/10000, 2) AS totalHoras,
                Turno.nCuenta
            FROM
                Turno
            JOIN
...
// Consulta SQL para obtener los registros combinados de ResumenSS y Completo
$sql = "
    SELECT r.idResumen, r.periodo, r.totalTurnos, r.totalHoras, s.nombreSS, s.fInicioSS, s.fFinSS, c.totalHorasCompl, c.periodoCompl
    FROM ResumenSS r
    INNER JOIN ServicioSocial s ON r.nCuenta = s.nCuenta
    LEFT JOIN Completo c ON r.nCuenta = c.nCuenta
    WHERE r.periodo = (
        SELECT MAX(periodo)
        FROM ResumenSS rr
        WHERE rr.nCuenta = r.nCuenta
    )
";
                        ...
                        <?php
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    echo "<td class='$rowClass'>" . $row["totalHoras"] . "</td>";
                                    echo '<td>';
                                    // Botón modificado
            ...
            <script>
                ...
                $(document).ready(function() {
                    $('#datatablesSimple tbody tr').each(function() {
                        var totalHoras = parseInt($(this).find('td:last-child').text());

                        if (totalHoras >= 20) {
                            $(this).addClass('table-success');
                        } else if (totalHoras >= 15 && totalHoras < 20) {
