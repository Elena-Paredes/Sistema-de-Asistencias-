...
if (isset($_SERVER["REQUEST_METHOD"]) && $_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["actualizar"])) {
    // Consulta SQL para borrar todos los registros de la tabla Mes
    $truncateQuery = "TRUNCATE TABLE Mes";

    // Ejecutar la consulta de borrado
    if ($conn->query($truncateQuery) === TRUE) {
        // Consulta SQL para insertar los datos actualizados en la tabla Mes
        $insertQuery = "INSERT INTO Mes (periodoMes, totalTurnosMes, totalHorasMes, nCuenta)
                        SELECT
                            CONCAT('Mes-', MONTH(FechaTurno)) AS periodoMes,
                            COUNT(*) AS totalTurnosMes,
                            TRUNCATE(SUM(TIMEDIFF(hSalida, hEntrada))/10000, 2) AS totalHorasMes,
                            Turno.nCuenta
                        FROM
                            Turno
                        JOIN
...
$sql = "SELECT r.idMes, r.periodoMes, r.totalTurnosMes, r.totalHorasMes, s.nombreSS, s.fInicioSS, s.fFinSS, c.totalHorasCompl, c.periodoCompl 
        FROM Mes r
            ...
            <div id="layoutSidenav_nav">
             ...
             <main>        
                    <div class="container-fluid px-4">
                        ...
                        <div class="card mb-4">
                            <div class="card-body">
                               Resumen de las horas laboradas del personal del Servicio Social durante el mes.  
                            </div>
                        </div>
                                        ...
                                        <tbody>
                                            ...
                                            if ($result->num_rows > 0) {
                                                while ($row = $result->fetch_assoc()) {
                                                    ...
                                                    $rowClass = ($totalHorasMes >= 80) ? "table-success" : (($totalHorasMes >= 60 && $totalHorasMes < 80) ? "table-warning" : "table-danger");
                                            
                                                    echo "<tr class='$rowClass'>";
                                                    echo "<td>" . $row["nombreSS"] . "</td>";
                                                    echo "<td>" . $row["fInicioSS"] . "</td>";
                                                    echo "<td>" . $row["fFinSS"] . "</td>";
                                                    echo "<td>" . $row["periodoMes"] . "</td>";
                                                    echo "<td class='$rowClass'>" . $row["totalHorasMes"] . "</td>";
                                                    echo '<td>';
                                                    // Botón modificado
            ...
            <script>
                ...
                $(document).ready(function() {
                    $('#datatablesSimple tbody tr').each(function() {
                        var totalHorasMes = parseInt($(this).find('td:last-child').text());

                        if (totalHorasMes >= 80) {
                            $(this).addClass('table-success');
                        } else if (totalHorasMes >= 79 && totalHorasMes < 60) {
